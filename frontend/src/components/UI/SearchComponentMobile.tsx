...
export default function SearchComponent({params, onHandleSearch}: SearchParams) {
  ...
  // Count elements that DON'T match the condition (valid/non-empty values)
  const filterCount = Object.values(searchParams).reduce((count, value) => count + (value !== undefined && value !== null && value !== '' && !(Array.isArray(value) && value.length === 0) ? 1 : 0), 0);
  ...
  return (
    <div>
      <Button className='flex items-center gap-2' onClick={() => setIsOpen(true)}>
        <FilterIcon />
        <span>Filters</span>
        {filterCount > 0 && <span className='font-semibold text-sm'>{filterCount}</span>}
      </Button>      
